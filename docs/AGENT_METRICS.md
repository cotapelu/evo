...
- **Test Typing**: Refactored `mock-factory.ts` to return typed `ExtensionAPI` and `ExtensionContext`. Updated plugin capability tests (git, dev, security-system) to use typed mocks. Refactored command tests: provider-command, metrics-command, copy-command, team-command using `vi.mocked()` and typed contexts. Progressively eliminating `as any` casts across test suite.
