...
- **Test Typing**: Refactored `mock-factory.ts` to return typed `ExtensionAPI` and `ExtensionContext`. Plugin capability tests (git, dev, security-system) now typed. Command tests (provider-command, metrics-command, copy-command, team-command) use `vi.mocked()`. Renderer test (todos-renderer) introduced typed RendererFn. Systematic elimination of `as any` casts continues.
