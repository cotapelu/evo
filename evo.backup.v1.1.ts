...
export class EvoAgent {
  ...
  private createEvolutionPlan(analysis: ReturnType<this['analyzeCurrentState']>): EvolutionPlan {
    ...
    // Generate rollback plan based on risk
    let rollbackPlan: string[] = [];
    if (riskLevel !== 'low') {
      rollbackPlan = [
        'Restore from latest backup',
        'Reset state to previous iteration',
        'Revert code changes manually'
      ];
      if (riskLevel === 'critical') {
        rollbackPlan.push('Terminate all child agents');
        rollbackPlan.push('Clear recent history');
      }
    }
  ...
  private generateChangesForOpportunity(opportunity: string): string[] {
    ...
    if (opportunity.includes('persistent memory') || opportunity.includes('file storage')) {
      changes.push('Add saveMemory()/loadMemory() with JSON serialization');
      changes.push('Store memory to disk on each iteration');
      changes.push('Add memoryPath config');
    }
    if (opportunity.includes('FileSystem')) {
      changes.push('Create FileSystem class wrapper');
      changes.push('Add methods: readFile, writeFile, exists, listFiles');
      changes.push('Add path validation and sandboxing');
    }
    if (opportunity.includes('replication')) {
      changes.push('Add spawnChild() method');
      changes.push('Track children in state.children');
      changes.push('Add maxChildren limit');
      changes.push('Add resource allocation for children');
    }
    if (opportunity.includes('messaging')) {
      changes.push('Define Message interface with ID, TTL, priority');
      changes.push('Add sendMessage() and broadcast()');
      changes.push('Store messages in state.messages');
      changes.push('Add message queue size limit');
    }
    if (opportunity.includes('goal')) {
      changes.push('Define Goal interface with dependencies');
      changes.push('Add createGoal(), updateGoal(), advanceGoal()');
      changes.push('Persist goals with memory');
      changes.push('Add goal dependency tracking');
    }
    if (opportunity.includes('logging')) {
      changes.push('Add log buffer and logPath config');
      changes.push('Implement flushLogs() method');
      changes.push('Write logs to file periodically');
      changes.push('Add log rotation');
    }
    if (opportunity.includes('health') || opportunity.includes('monitoring')) {
      changes.push('Add performHealthCheck() method');
      changes.push('Add updateHealthCheck() in iteration');
      changes.push('Track memory pressure and failures');
      changes.push('Implement triggerAutoRecovery()');
    }
    if (opportunity.includes('sandbox') || opportunity.includes('security')) {
      changes.push('Add validatePath() and isWithinSandbox()');
      changes.push('Enhance FileSystem with allowedPaths');
      changes.push('Block dangerous operations');
      changes.push('Add security config section');
    }
    if (opportunity.includes('statistics')) {
      changes.push('Add stats tracking to state');
      changes.push('Increment counters on events');
      changes.push('Report stats in logs');
    }
    return changes;
  }
