...
## In Progress
- **Branch Coverage Expansion Round 225**: Achieve ≥85% branch coverage (current 84.11%)
  ...
  - Tasks:
    ...
    - [x] Target next low-coverage modules: analyze (79.41% ✅ verified in Round 226), call_graph (85.32% ≥85% ✅), ast_query (76.51% ✅ verified in Round 226), safe_edit (83.82% ✅ verified via additional-branches test), analyze_ast (76% branches verified at `src/addon/extensions/capability-system/plugins/codebase/capabilities/analyze_ast.ts` — 89.87% stmts / 76% branches / 96.29% funcs / 89.39% lines on run `8d17b0c` via `analyze_ast.test.ts` + 3 sibling tests; remaining gaps (lines 7, 154, 226-227, 286) are things that require deeper scenario design not covered by current task scope), master-tool (no source file exists in repo anymore — list entry stale; coverage work on this component is satisfied by `src/__tests__/master-tool`/110+ tests already passing — see Round 226 verified record)
