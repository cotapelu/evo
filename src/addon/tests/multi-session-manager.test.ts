...
describe('MultiSessionManager', () => {
  ...
  describe('createChild()', () => {
    ...
    it('should respect maxSessions limit', async () => {
      ...
      await expect(limitedManager.createChild({ name: 'child-2' })).rejects.toThrow(
        'Max sessions limit'
      );
    });
  ...
  describe('dispose()', () => {
    ...
    it('should not dispose parent unless disposeRuntime=true', async () => {
      await expect(manager.dispose(manager.getRoot()!.id)).rejects.toThrow(
        'Cannot dispose parent session'
      );
    });
