...
export function createMasterTool(options: any = {}): ToolDefinition {
  ...
  return {
    ...
    promptSnippet: "master_tool_1({ command: '<name>', args: {...} })",
    promptGuidelines: [
      "The master_tool_1 provides access to many specialized commands.",
      "",
      "**Structure**:",
      "  master_tool_1({",
      ...
      "",
      "**Discover commands**:",
      "  • List all: master_tool_1({ command: 'list', args: {} })",
      "  • By category: master_tool_1({ command: 'list.grep', args: { category: 'git' } })",
      "  • Search: master_tool_1({ command: 'list.grep', args: { query: 'test' } })",
      "  • Get help: master_tool_1({ command: 'help', args: { command: 'git.status' } })",
      "",
      "**Examples**:",
      "  • Git status:",
      "    master_tool_1({ command: 'git.status', args: {} })",
      "",
      "  • Run tests:",
      "    master_tool_1({ command: 'dev.test', args: { files: ['src/'], coverage: true } })",
      "",
      "  • System info:",
      "    master_tool_1({ command: 'system.info', args: { detailed: true } })",
      "",
