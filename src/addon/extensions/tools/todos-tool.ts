...
function createTodoTool(api: ExtensionAPI): ToolDefinition<any, TodoToolDetails> {
  ...
  return {
    ...
    promptSnippet: "todos_1({ OPERATION: {...} }). All params are OBJECTS (not JSON strings). Ops: add_phase, add_task, update, remove_task, delete, list.",
    promptGuidelines: [
      "RULE: Operation is the KEY. todos_1({ OPERATION: params }), NOT { operation: name, params: {...} }",
      "",
      "TEMPLATES:",
      "• add_phase: todos_1({ add_phase: { name: 'Phase', tasks: [{ content: 'Task' }] } })",
      "• add_task:  todos_1({ add_task: { phase: 'phase-1', content: 'Task' } })",
      "  ⚠️ phase must be ID (phase-1), NOT name",
      "• update:    todos_1({ update: { id: 'task-5', status: 'in_progress' } })",
      "  Batch:    todos_1({ update: { ids: ['task-1','task-2'], status: 'completed' } })",
      "  status: pending|in_progress|completed|abandoned",
      "• remove_task: todos_1({ remove_task: { id: 'task-3' } })",
      "• delete: todos_1({ delete: {} })",
      "• list: todos_1({ list: {} })",
      "",
      "AUTO-RULES:",
      ...
      "• Task IDs are auto: task-1, task-2... Find them by running todos_1({ list: {} })",
    ],
    parameters: {},
