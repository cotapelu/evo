...
export function registerMemoryTool(api: ExtensionAPI): void {
  ...
  const tool: ToolDefinition = {
    ...
    promptSnippet: "memory_1({ action: '<action>', ...params })",
    promptGuidelines: [
      "Add: memory_1({ action: 'add', text: 'Important fact', tags?: ['tag1', 'tag2'] })",
      "List: memory_1({ action: 'list' })",
      "Get: memory_1({ action: 'get', id: <number> })",
      "Delete: memory_1({ action: 'delete', id: <number> })",
      "Clear: memory_1({ action: 'clear' })",
      "Search: memory_1({ action: 'search', query: 'text' })",
      ...
      "Use memory_1.search to retrieve stored information when the user asks 'what did we say about X', 'do you remember', or references past info.",
