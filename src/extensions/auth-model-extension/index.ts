...
// ============================================================================
export function createListCredentialsTool(): ToolDefinition {
	return {
		name: "auth.list",
		label: "Auth: List",
		description: "List stored credentials (provider IDs)",
		parameters: {},
		async execute(toolCallId, params, signal, onUpdate, ctx: any) {
			try {
				...
				const lines = credentials.map((c: any) => `  ${c.provider}: ${c.id.substring(0, 8)}...`);
				return {
...
// ============================================================================
export function createClearCredentialTool(): ToolDefinition {
	return {
		name: "auth.clear",
		label: "Auth: Clear",
...
// ============================================================================
export function createListModelsTool(): ToolDefinition {
	return {
		name: "models.list",
		label: "Models: List",
		...
		async execute(toolCallId, params, signal, onUpdate, ctx: any) {
			try {
				...
				const { provider } = params as any;
				const models = provider ? allModels.filter((m: any) => m.provider === provider) : allModels;
				...
				const lines = models.map((m: any) =>
					`  ${m.id} | ${m.provider} | ctx:${m.contextWindow} | ${m.reasoning ? '🧠' : ''}`
				);
				return {
...
// ============================================================================
export function createGetModelTool(): ToolDefinition {
	return {
		name: "models.get",
		label: "Models: Get",
...
// ============================================================================
function registerAuthModelCommands(api: ExtensionAPI): void {
	api.registerCommand("auth.status", {
		...
		handler: async (_args: string, ctx: any) => {
			...
			const lines = [
				...
				...credentials.map((c: any) => `  ${c.provider} (${c.id.substring(0, 8)}...)`),
			];
...
// Main Export
// ============================================================================
export function registerAuthModelExtension(api: ExtensionAPI): void {
