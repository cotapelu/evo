...
describe('team_run tool', () => {
  ...
  describe('parameter validation', () => {
    it('should reject call reference strings', async () => {
    ...
    it('should accept valid object with tasks', async () => {
  ...
  describe('missing runtime context', () => {
    it('should return error when ctx.runtime missing', async () => {
      const badCtx: any = {};
      const result: any = await tool.execute(toolCallId, { tasks: ['test'] }, undefined, undefined, badCtx);
      expect(result.isError).toBe(true);
