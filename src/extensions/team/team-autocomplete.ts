 ...
 */
function detectTeamRunContext(lines: string[], cursorLine: number, cursorCol: number): { field: string; token: string } | null {
 ...
 */
function filterRoles(query: string): AutocompleteItem[] {
	if (!query.trim()) {
 ...
 */
function suggestTeamIds(query: string): AutocompleteItem[] {
	try {
 ...
 */
function suggestTeamSize(query: string): AutocompleteItem[] {
	const sizes = ["2", "3", "4"];
	if (!query.trim()) {
 ...
 */
export function createTeamRunAutocompleteProvider(
