  ...
  describe('Command Execution', () => {
    ...
    test('export command calls correct Python module', async () => {
      ...
      expect(ctx.exec).toHaveBeenCalledWith(
        'python3',
        expect.arrayContaining(['-m', 'kicad.sch.export', 'schematic.kicad_sch', '--format', 'pdf']),
    ...
    test('erc command includes severity if provided', async () => {
      ...
      expect(ctx.exec).toHaveBeenCalledWith(
        'python3',
        expect.arrayContaining(['-m', 'kicad.sch.erc', 'schematic.kicad_sch', '--severity', 'error']),
        { cwd: '/test/project', signal: undefined }
      );
    });
    ...
    test('field_edit passes field_name and field_value', async () => {
      ...
      expect(ctx.exec).toHaveBeenCalledWith(
        'python3',
        expect.arrayContaining(['-m', 'kicad.sch.field_edit', 'schematic.kicad_sch', '--field-name', 'Value', '--field-value', '10k']),
        { cwd: '/test/project', signal: undefined }
      );
    });
    ...
    test('replace_fonts passes old and new font', async () => {
      ...
      expect(ctx.exec).toHaveBeenCalledWith(
        'python3',
        expect.arrayContaining(['-m', 'kicad.sch.replace_fonts', 'schematic.kicad_sch', '--old-font', 'OldFont', '--new-font', 'NewFont']),
        { cwd: '/test/project', signal: undefined }
      );
    });
