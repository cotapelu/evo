...
function createMockContext(sessionManager?: any): ExtensionContext {
  return {
...
describe('Memory Tool', () => {
  ...
  describe('Execute actions', () => {
    ...
    test('list: returns all memories', async () => {
      ...
      expect(lines.some((l: string) => l.includes('#1: a'))).toBe(true);
      expect(lines.some((l: string) => l.includes('#2: b'))).toBe(true);
    });
