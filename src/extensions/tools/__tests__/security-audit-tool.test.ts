...
import { registerSecurityAuditTool } from '../security-audit-tool.js';
...
describe('Security Audit Tool', () => {
  ...
  test('executes npm audit and scans for issues', async () => {
    ...
    expect(result.details?.issues.some((i: any) => i.check === 'npm-audit')).toBe(true);
  });
  ...
  test('handles no vulnerabilities from npm audit', async () => {
    ...
    const auditIssues = result.details?.issues.filter((i: any) => i.check === 'npm-audit');
  ...
  test('detects secret patterns in files', async () => {
    ...
    const highSecrets = result.details?.issues.filter((i: any) => i.check === 'secret-scan' && i.severity === 'high');
  ...
  test('checks package-lock.json', async () => {
    ...
    const lockIssue = result.details?.issues.find((i: any) => i.check === 'package-lock');
  ...
  test('detects insecure patterns', async () => {
    ...
    const highInsecure = result.details?.issues.filter((i: any) => i.check === 'insecure-pattern' && i.severity === 'high');
    expect(highInsecure?.length).toBe(0);
    const mediumInsecure = result.details?.issues.filter((i: any) => i.check === 'insecure-pattern' && i.severity === 'medium');
