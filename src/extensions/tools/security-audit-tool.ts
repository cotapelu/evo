...
function createSecurityAuditTool(api: ExtensionAPI): ToolDefinition<any, any> {
  return {
    name: 'security-audit',
    ...
    async execute(toolCallId, params, signal, _onUpdate, ctx) {
      ...
      // 1. npm audit
      const auditExecOptions: ExecOptions = { cwd };
      if (signal) auditExecOptions.signal = signal;
      try {
        ...
        if (auditRes.code === 0) {
          ...
          try {
            ...
            if (auditJson.vulnerabilities) {
              for (const [pkg, vuln] of Object.entries(auditJson.vulnerabilities as any)) {
                const v = vuln as any;
                issues.push({
...
async function scanForSecrets(cwd: string, signal?: AbortSignal): Promise<SecurityIssue[]> {
  const issues: SecurityIssue[] = [];
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          if (entry.name === 'node_modules' || entry.name === '.git' || entry.name === 'dist' || entry.name === 'coverage' || entry.name === 'build' || entry.name === '__tests__' || entry.name === 'llm-context' || entry.name === 'docs') continue;
          await walkDir(full);
...
function checkPackageLock(cwd: string): SecurityIssue | null {
...
async function scanInsecurePatterns(cwd: string, signal?: AbortSignal): Promise<SecurityIssue[]> {
  const issues: SecurityIssue[] = [];

  const patterns: { regex: RegExp; message: string; severity: 'high' | 'medium' | 'low' }[] = [
...
export function registerSecurityAuditTool(api: ExtensionAPI): void {
