...
function createCoverageHistoryTool(): ToolDefinition<any, any> {
  return {
    ...
    renderResult(result: any, options: { expanded: boolean; isPartial: boolean }, theme: any) {
      ...
      if (result.isError) return new Text(th.fg('error', 'No history'), 0, 0);
...
export function registerCoverageHistoryTool(api: ExtensionAPI): void {
