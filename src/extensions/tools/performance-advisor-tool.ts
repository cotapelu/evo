...
interface PerformanceAdvisorDetails {
  tools: ToolStats[];
  suggestions: string[];
...
function computeStats(metrics: Array<{ toolName: string; duration: number; success: boolean; timestamp: number }>): PerformanceAdvisorDetails {
  const map = new Map<string, { sum: number; count: number; max: number; errors: number; last: number }>();
  ...
  for (const m of metrics) {
    ...
    if (!m.success) existing.errors += 1;
...
export function registerPerformanceAdvisorTool(api: ExtensionAPI) {
  api.registerTool({
    ...
    async execute(toolCallId, params, signal, onUpdate, ctx) {
      ...
      let content: string;
      let details: PerformanceAdvisorDetails;

      try {
        const data = await readFile(path, 'utf-8');
        const lines = data.split('\n').filter(l => l.trim() !== '');
        const metrics: Array<{ toolName: string; duration: number; success: boolean; timestamp: number }> = [];
