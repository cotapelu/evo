      ...
      try {
        ...
        if (op === "info") {
          ...
          const text = [
            ...
            `Messages: ${entries.filter((e: any) => e.type === 'message').length}`,
          ].join("\n");
          return {
        ...
        if (op === "graph") {
          ...
          if (branch.length === 0) return { content: [{ type: "text", text: "(empty branch)" }], details: {} };
          const lines = branch.slice(-20).map((e: any) => `${e.id.substring(0, 6)} ${e.type}`);
          return {
...
// Settings Tool
// ============================================================================
export function createSettingsTool(): ToolDefinition {
  return {
    name: "sdk.settings",
    label: "SDK: Settings",
    ...
    async execute(toolCallId, params, signal, onUpdate, ctx) {
      ...
      try {
        ...
        if (action === "get") {
          const key = p.key;
          if (!key) return { content: [{ type: "text", text: "❌ key required" }], isError: true, details: { error: "missing_key" } };
...
// ============================================================================
export function createResourceTool(): ToolDefinition {
  return {
    name: "sdk.resources",
    label: "SDK: Resources",
    ...
    async execute(toolCallId, params, signal, onUpdate, ctx) {
      try {
        ...
        const paths = resources.map((r: any) => r.path).slice(0, 20);
...
// Quick Command
// ============================================================================
function registerQuickCommand(api: ExtensionAPI): void {
...
// Main
// ============================================================================
export function registerSdkIntegrationExtension(api: ExtensionAPI): void {
