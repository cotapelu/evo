...
/**
 * Evo Configuration Manager
 * Handles persistent user configuration (~/.evo/config.json)
...
export const CONFIG_DIR_NAME = ".evo";
...
export function getAgentDir(): string {
	const envDir = process.env.EVO_AGENT_DIR;
	if (envDir) {
 ...
 */
export function loadConfig(cliOverrides?: Partial<EvoConfig>): EvoConfig {
	...
	let fileConfig: EvoConfig = { ...DEFAULT_CONFIG };
 ...
 */
export async function saveConfig(config: EvoConfig): Promise<void> {
	const configDir = getConfigDir();
	const configPath = getConfigFilePath();
