...
interface Tool {
	name: ToolName;
	downloadUrl?: (platform: string, arch: string) => string;
	version: string;
}
 ...
 */
export async function ensureTool(name: ToolName): Promise<string> {
	...
	throw new Error(
		`Tool '${name}' not found. Please install it manually:\n` +
 ...
 */
export function checkToolInstalled(name: ToolName): boolean {
	try {
 ...
 */
export function getToolVersion(name: ToolName): string | null {
	try {
		const result = spawnSync(name, ['--version'], { encoding: 'utf8' });
		if (result.status === 0) {
