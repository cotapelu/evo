...
export class EvoAgent {
  ...
  private async expandCapabilities(): Promise<void> {
    ...
    const candidates = [
      'deployment', 'workerpool', 'asttransformation', 'orchestration', 'performancetuning',
      'autorecovery', 'distributedcoordination', 'caching', 'loadbalancing', 'monitoring', 'pluginsystem', 'eventdriven', 'metricscollection', 'dynamicreconfiguration'
    ];
    
    for (const cap of candidates) {
      const normalized = cap.toLowerCase().replace(/[^a-z]/g, '');
      if (!current.includes(normalized) && this.state.level >= 10) {
        ...
        if (this.state.capabilities.length >= 30) break; // Cap at 30 for now
      }
    }
  }

  private formatCapabilityName(cap: string): string {
    // Convert 'workerpool' -> 'worker-pool'
