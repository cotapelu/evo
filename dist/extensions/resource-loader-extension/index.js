...
// Scanning
// ============================================================================
function scanDocumentation(cwd) {
    const additionalFiles = [];
    const isDocFile = (filename) => {
        return filename.endsWith(".md") || filename.endsWith(".MD");
    };
    function computePriority(path) {
        const lower = path.toLowerCase();
        if (lower.includes("agents.md") || lower.includes("claude.md"))
            return 3;
...
// Event Handlers
// ============================================================================
function registerEventHandlers(api) {
    ...
    return {
        ...
        async execute(toolCallId, params, signal, _onUpdate, ctx) {
            try {
                ...
                const lines = allFiles.map((f) => `  ${f.path} (${f.content.length} bytes)`).join("\n");
                return {
                    content: [{ type: "text", text: `Loaded resources (${allFiles.length}):\n${lines}` }],
                    details: { count: allFiles.length, files: allFiles.map((f) => f.path) },
                    isError: false,
                };
            }
            catch (e) {
...
function registerResourceCommands(api) {
    api.registerCommand("resources.list", {
        ...
        handler: async (args, ctx) => {
            ...
            const lines = allFiles.slice(0, 20).map((f) => f.path);
