 ...
 */
function detectSessionManagerContext(lines, cursorLine, cursorCol) {
 ...
 */
async function suggestSessionFiles(token) {
    try {
        if (!sessionCwd)
            return [];
            ...
            try {
                ...
                for (const entry of entries) {
                    ...
                    if (entry.isDirectory()) {
                        if (['node_modules', '.git', 'dist', 'coverage', '.next', 'build'].includes(entry.name))
                            continue;
 ...
 */
async function suggestEntryIds(token) {
    try {
        if (!sessionManager)
            return [];
        ...
        const matches = fuzzyFilter(entryIds, token, (id) => id);
 ...
 */
export function createSessionManagerAutocompleteProvider(current, getSuggestionsCache) {
    return {
 ...
 */
export function registerSessionManagerAutocomplete(api) {
    const getSuggestionsCache = new Map();
