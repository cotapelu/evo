...
async function measure(label, fn, iterations = 3) {
...
function createBenchRunTool(api) {
    return {
        name: 'bench.run',
        label: 'Bench: Run',
        ...
        async execute(toolCallId, params, signal, onUpdate, ctx) {
            ...
            try {
                const gitResult = await measure('git status --porcelain', () => api.exec('git', ['status', '--porcelain'], { cwd: ctx.cwd, signal }));
...
export function registerBenchmarkExtension(api) {
