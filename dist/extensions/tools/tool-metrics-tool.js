...
function createToolMetricsTool() {
    return {
        name: 'tool-metrics',
        label: 'Tool Metrics',
        ...
        renderResult(result, options, theme) {
            ...
            if (result.isError)
                return new Text(th.fg('error', 'Error'), 0, 0);
            const runs = result.details?.totalRuns || 0;
            return new Text(th.fg('accent', `📈 ${runs} records`), 0, 0);
        },
    };
}
export function registerToolMetricsTool(api) {
    api.registerTool(createToolMetricsTool());
}
//# sourceMappingURL=tool-metrics-tool.js.map