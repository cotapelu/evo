...
// ============================================================================
function getProjectTodoFilePath(cwd) {
    return join(cwd, ".pi", "agent", "todos.json");
}
async function loadTodoFromFile(cwd) {
...
async function saveTodoToFile(cwd, todo) {
    const filePath = getProjectTodoFilePath(cwd);
...
function buildPhaseFromInput(input, phaseId, nextTaskId) {
    const tasks = [];
    let tid = nextTaskId;
...
function getNextIds(phases) {
    ...
    for (const phase of phases) {
        ...
        if (phaseMatch) {
            ...
            if (Number.isFinite(value) && value > maxPhaseId)
                maxPhaseId = value;
        }
        for (const task of phase.tasks) {
            ...
            if (Number.isFinite(value) && value > maxTaskId)
                maxTaskId = value;
        }
    }
...
function normalizeParams(params) {
    ...
    if (normalized.add_phase && typeof normalized.add_phase === "object") {
        const addPhase = normalized.add_phase;
    ...
    if (normalized.add_phase && typeof normalized.add_phase === "object") {
        ...
        if (addPhase.tasks && typeof addPhase.tasks === "string") {
            try {
                ...
                addPhase.tasks = addPhase.tasks.split(",").map((s) => ({ content: s.trim() }));
    ...
    if (normalized.delete !== undefined && typeof normalized.delete === "string") {
        try {
            ...
            throw new Error(`delete must be an object, not a string. Error parsing: ${e instanceof Error ? e.message : String(e)}`);
        }
    }
    ...
    ["add_task", "update", "remove_task", "delete"].forEach(op => {
        if (normalized[op] && typeof normalized[op] === "string") {
            try {
                ...
                throw new Error(`${op} must be an object, not a string. Error parsing: ${e instanceof Error ? e.message : String(e)}`);
            }
        }
    });
    return normalized;
...
function applySingleOp(file, params) {
    const errors = [];
...
function formatSummary(phases, errors) {
...
// ============================================================================
function countOperations(params) {
    let count = 0;
    if (params.add_phase)
        count++;
    if (params.add_task)
...
function applyOp(phases, nextTaskId, nextPhaseId, params) {
    const file = { phases, nextTaskId, nextPhaseId };
...
class TodoState {
    ...
    nextTaskId = 1;
    nextPhaseId = 1;
    storageType = "file"; // NEW: track storage type
    listeners = new Set();
    subscribe(listener) { this.listeners.add(listener); return () => this.listeners.delete(listener); }
    notify() { for (const l of this.listeners)
        l(); }
    async loadFromFile(cwd) {
    ...
    reconstructFromEntries(entries) {
        ...
        for (let i = entries.length - 1; i >= 0; i--) {
            ...
            if (e.type !== "message")
                continue;
            const m = e.message;
            if (m.role !== "toolResult" || (m.toolName !== "todos" && m.toolName !== "todo_write"))
                continue;
            const d = m.details;
            if (d?.phases) {
    ...
    addTask(phaseId, content, notes, details) {
...
function formatTodoLineExtension(item, theme, prefix) {
...
function renderTodosCall(args, theme) {
...
function renderTodosResult(result, options, theme) {
    ...
    const lines = [theme.fg("toolTitle", `Todos: ${allTasks.length} tasks`)];
    for (const phase of phases) {
        if (phases.length > 1)
    ...
    return {
        ...
        async execute(toolCallId, params, _signal, _onUpdate, ctx) {
            ...
            try {
                ...
                if (opCount === 0)
                    errors.push("No operation specified. Use: add_phase, add_task, update, remove_task, delete, or list");
                if (opCount > 1)
                    errors.push("Multiple operations detected. Use only ONE operation per call.");
                const opName = getOperationName(p);
                const op = opName === "unknown" ? null : opName;
                if (!op) {
                    errors.push("No operation. Use: add_phase, add_task, update, remove_task, list");
                }
...
// Export
// ============================================================================
export function registerTodosTool(api) {
    api.registerTool(createTodoTool(api));
}
//# sourceMappingURL=todos-tool.js.map