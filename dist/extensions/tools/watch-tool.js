...
function createWatchTool(api) {
    const tool = {
        ...
        async execute(toolCallId, params, signal, onUpdate, ctx) {
            ...
            const commands = Array.isArray(p.commands) ? p.commands : ['code-health', 'test --coverage'];
            ...
            let logLines = [`🔍 Watching ${watchPaths.length} paths (${watchPaths.join(', ')})`];
            let lastRun = null;
            ...
            async function walkAndWatch(dir) {
                try {
        ...
        renderResult(result, options, theme) {
            ...
            if (options.isPartial)
                return new Text(th.fg('warning', 'Initializing watch...'), 0, 0);
            const details = result.details || {};
            if (details.status === 'stopped')
                return new Text(th.fg('dim', 'Watch stopped'), 0, 0);
...
//# sourceMappingURL=watch-tool.js.map