...
const bashToolCache = new WeakMap();
function getBashTool(ctx) {
 ...
 */
async function executeUniversal(_toolCallId, params, signal, onUpdate, ctx) {
    const { action } = params;
    if (!action) {
    ...
    try {
        ...
        const result = await bashTool.execute(_toolCallId, { command }, signal, onUpdate, ctx);
 ...
 */
function renderUniversal(result, options, theme) {
 ...
 */
export function registerUniversalTool(api) {
    const tool = {
        name: "universal",
        label: "Universal Tool",
        description: "Multi-purpose tool with registry-based actions. Supports: echo, system_info, date, uuid, random, calc. Easily extensible.",
        promptSnippet: "Use `universal` tool with `action` parameter to perform various tasks.",
        promptGuidelines: [
...
//# sourceMappingURL=universal-tool.js.map