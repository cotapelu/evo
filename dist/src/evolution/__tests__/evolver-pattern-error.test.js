...
describe('Evolver – Pattern Error Resilience', () => {
    ...
    beforeAll(() => {
        ...
        if (pattern)
            originalFix = pattern.fix;
    });
    afterAll(() => {
        // Restore original fix
        const pattern = patterns.find(p => p.id === 'trailing-whitespace');
        if (pattern && originalFix)
            pattern.fix = originalFix;
    });
    beforeEach(async () => {
    ...
    it('should continue evolution when a pattern.fix throws, logging warning with pattern ID', async () => {
        ...
        const pattern = patterns.find(p => p.id === 'trailing-whitespace');
        if (!pattern)
            throw new Error('trailing-whitespace pattern not found');
        ...
        expect(warningCalls.some(msg => typeof msg === 'string' &&
            msg.includes('Pattern trailing-whitespace failed on') &&
            msg.includes('simulated pattern failure'))).toBe(true);
        warnSpy.mockRestore();
    });
});
//# sourceMappingURL=evolver-pattern-error.test.js.map