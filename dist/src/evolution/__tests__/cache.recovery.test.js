...
describe('FileCache – Recovery & Atomicity', () => {
    let cacheDir;
    let cache;
    const manifestDir = (dir) => join(dir, '.evo-cache');
    const manifestPath = (dir) => join(dir, '.evo-cache', 'manifest.json');
    const backupPath = (dir) => join(dir, '.evo-cache', 'manifest.json.bak');
    beforeEach(() => {
        cacheDir = join(os.tmpdir(), `evo-cache-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`);
        mkdirSync(cacheDir, { recursive: true });
        // Remove any existing cache files
        try {
            rmSync(manifestDir(cacheDir), { recursive: true, force: true });
        }
        catch { }
...
//# sourceMappingURL=cache.recovery.test.js.map