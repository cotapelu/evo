...
export function registerMemoryTool(api) {
    const reconstructState = async (ctx) => {
    ...
    const tool = {
        ...
        renderCall(args, theme, _context) {
            ...
            if (args.text)
                text += ` ${th.fg("dim", `"${args.text.substring(0, 30)}${args.text.length > 30 ? "..." : ""}"`)}`;
            if (args.id !== undefined)
                text += ` ${th.fg("accent", `#${args.id}`)}`;
            if (args.tags)
                text += ` ${th.fg("warning", `[${args.tags.length} tags]`)}`;
        ...
        renderResult(result, options, theme, _context) {
            ...
            switch (action) {
                ...
                case "list": {
                    ...
                    if (count === 0)
                        return new Text(th.fg("dim", "No memories"), 0, 0);
