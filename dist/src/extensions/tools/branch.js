...
function renderBranchCall(args, theme) {
    const action = args.action || 'unknown';
    const text = `${theme.fg("toolTitle", theme.bold("branch"))} ${theme.fg("muted", action)}`;
    return new Text(text, 0, 0);
}
function renderBranchResult(result, options, theme) {
    ...
    const lines = [];
    if (details.leaves)
        lines.push(`${theme.fg("accent", "Leaves")}: ${details.leaves.length}`);
    if (details.labels)
        lines.push(`${theme.fg("accent", "Labels")}: ${details.labels.length}`);
    if (details.entry)
        lines.push(`${theme.fg("accent", "Entry")}: ${details.entry.id} (${details.entry.type})`);
    if (details.leaf)
        lines.push(`${theme.fg("accent", "Leaf")}: ${details.leaf.id}`);
    if (details.branch)
        lines.push(`${theme.fg("accent", "Branch path")}: ${details.branch.length} entries`);
    if (details.tree)
        lines.push(`${theme.fg("accent", "Tree")}: ${details.tree.length} root nodes`);
    if (lines.length === 0)
...
function createBranchTool() {
    return {
        ...
        async execute(toolCallId, params, _signal, _onUpdate, ctx) {
            ...
            const allowed = ['list_leaves', 'list_labels', 'get_entry', 'get_leaf', 'get_branch', 'get_tree'];
            ...
            try {
                switch (action) {
                    case 'list_leaves': {
                        ...
                        const childIds = new Set(entries.map(e => e.parentId).filter((id) => id !== null));
...
export function registerBranchTool(api) {
