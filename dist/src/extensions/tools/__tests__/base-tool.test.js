...
describe('Base Stateful Tool', () => {
    ...
    test('renderCall and renderResult passed through', () => {
        const renderCall = jest.fn().mockReturnValue(undefined);
        const renderResult = jest.fn().mockReturnValue(undefined);
        const tool = createStatefulTool({
            name: 'rendered',
            label: 'Rendered',
        ...
        const ctx = { args: {}, toolCallId: '1', invalidate: () => { }, lastComponent: null };
        tool.renderCall?.(args, theme, ctx);
        expect(renderCall).toHaveBeenCalledWith(args, theme, ctx);
        tool.renderResult?.({}, { expanded: false, isPartial: false }, theme, ctx);
