...
describe('KiCad PCB Tool', () => {
    ...
    describe('Command Execution', () => {
        ...
        test('plot command calls correct Python module with layers', async () => {
            ...
            expect(ctx.exec).toHaveBeenCalledWith('python3', expect.arrayContaining([
        ...
        test('export command with format', async () => {
            ...
            expect(ctx.exec).toHaveBeenCalledWith('python3', expect.arrayContaining(['-m', 'kicad.pcb.export', 'board.kicad_pcb', '--format', 'step']), { cwd: '/test/project', signal: undefined });
        });
        test('route command with start/end/layer', async () => {
            ...
            expect(ctx.exec).toHaveBeenCalledWith('python3', expect.arrayContaining(['-m', 'kicad.pcb.route', 'board.kicad_pcb', '--start', 'U1-1', '--end', 'U2-1', '--layer', 'F.Cu']), { cwd: '/test/project', signal: undefined });
        });
        test('length_tuning with array of tracks', async () => {
            ctx.exec = jest.fn(async () => ({ stdout: 'ok', stderr: '', code: 0 }));
            const result = await tool.execute('call', {
                command: 'length_tuning',
                args: { input: 'board.kicad_pcb', target_length: 100, tracks: ['U1-1', 'U1-2'] }
            }, undefined, undefined, ctx);
            expect(ctx.exec).toHaveBeenCalledWith('python3', expect.arrayContaining(['-m', 'kicad.pcb.length_tuning', 'board.kicad_pcb', '--target-length', '100', '--track', 'U1-1', '--track', 'U1-2']), { cwd: '/test/project', signal: undefined });
        });
        test('tie command with pins array', async () => {
            ...
            expect(ctx.exec).toHaveBeenCalledWith('python3', expect.arrayContaining(['-m', 'kicad.pcb.tie', 'board.kicad_pcb', '--pin', 'U1-1', '--pin', 'U2-1']), { cwd: '/test/project', signal: undefined });
        });
...
//# sourceMappingURL=kicad-pcb.test.js.map