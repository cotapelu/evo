...
describe('Code Health Tool', () => {
    ...
    test('audit: handles check failure', async () => {
        api.exec.mockImplementation(async (cmd, args) => {
        ...
        const lintCheck = result.details?.checks.find((c) => c.name === 'lint');
    ...
    test('audit: handles exec exception', async () => {
        ...
        expect(result.isError).toBe(true);
        const buildCheck = result.details?.checks.find((c) => c.name === 'build');
    ...
    test('handles unknown check names gracefully by filtering', async () => {
        api.exec.mockImplementation(async () => ({ stdout: 'OK', stderr: '', code: 0 }));
        const ctx = createMockContext();
        const result = await tool.execute('1', { checks: ['nonexistent'] }, undefined, undefined, ctx);
        expect(result.isError).toBe(true);
        expect(result.content[0].text).toContain('No valid checks');
    });
});
//# sourceMappingURL=code-health-tool.test.js.map