    ...
    describe(`kicad_sch.${cmd.name}`, () => {
        ...
        describe('execute', () => {
            it('builds correct python command', async () => {
                const args = {};
                for (const f of cmd.required)
                    args[f] = 'test.kicad_sch';
                ...
                const execCall = ctx.exec.mock.calls[0];
                const pythonExe = execCall[0];
                const pythonArgs = execCall[1];
            ...
            it('propagates exec errors', async () => {
                const args = { input: 'test.kicad_sch' };
...
//# sourceMappingURL=kicad-sch-commands.test.js.map