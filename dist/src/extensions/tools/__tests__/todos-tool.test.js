...
import { existsSync, mkdirSync, rmSync, readFileSync, promises as fs } from 'fs';
...
function createMockContext(custom) {
    return {
        cwd: process.cwd(),
        sessionManager: {
            getBranch: jest.fn(() => []),
...
describe('Todos Tool – Isolation & Concurrency', () => {
    ...
    beforeEach(async () => {
        ...
        try {
            rmSync(join(todosDir, 'todos.json'));
        }
        catch { }
    });
    afterEach(async () => {
        try {
            await fs.rm(join(todosDir, 'todos.json'));
        }
        catch { }
        try {
            await fs.rm(join(todosDir, 'todos.json.bak'));
        }
        catch { }
        try {
            await fs.rm(todosDir, { recursive: true, force: true });
        }
        catch { }
    });
    ...
    test('delete: removes entire phase', async () => {
        ...
        const list1 = await tool.execute('2', { list: {} }, undefined, undefined, ctx);
        const phaseId = list1.details.phases[0].id;
        await tool.execute('3', { delete: { phase: phaseId } }, undefined, undefined, ctx);
        const list2 = await tool.execute('4', { list: {} }, undefined, undefined, ctx);
    ...
    test('auto_normalize: only one in_progress task per phase (keeps first, demotes later)', async () => {
        ...
        const t1 = finalTasks.find((t) => t.id === task1Id);
        const t2 = finalTasks.find((t) => t.id === task2Id);
...
//# sourceMappingURL=todos-tool.test.js.map