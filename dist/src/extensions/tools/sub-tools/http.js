...
export async function executeHttp(args, cwd, signal, ctx) {
    const { method = "GET", url, headers = {}, body, timeout = 30, insecure = false, user, verbose = false, } = args;
    try {
        const curlArgs = [];
        if (method !== "GET")
        ...
        return { content: [{ type: "text", text: `HTTP error: ${error.message}` }], details: undefined, isError: true };
    }
}
