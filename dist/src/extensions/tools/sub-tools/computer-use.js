 ...
 */
export async function executeLs(args, cwd, signal, ctx) {
    const { path, recursive = false, all = false } = args;
    try {
        const lsArgs = [];
        if (all)
            lsArgs.push("-la");
        ...
        return { content: [{ type: "text", text: `ls error: ${error.message}` }], details: undefined, isError: true };
    }
}
/**
 * Find files by pattern (find)
 */
export async function executeFind(args, cwd, signal, ctx) {
    const { pattern, path = cwd, maxDepth } = args;
    try {
        const findArgs = [path];
        if (maxDepth)
        ...
        return { content: [{ type: "text", text: `find error: ${error.message}` }], details: undefined, isError: true };
    }
}
/**
 * Search file contents (grep)
 */
export async function executeGrep(args, cwd, signal, ctx) {
    const { pattern, path = cwd, include, exclude, ignoreCase = false } = args;
    try {
        const grepArgs = [];
        if (ignoreCase)
            grepArgs.push("-i");
        ...
        return { content: [{ type: "text", text: `grep error: ${error.message}` }], details: undefined, isError: true };
    }
}
/**
 * Read file contents with optional offset/limit
 */
export async function executeRead(args, cwd, signal, ctx) {
    ...
    try {
        ...
        return { content: [{ type: "text", text: `read error: ${error.message}` }], details: undefined, isError: true };
    }
}
