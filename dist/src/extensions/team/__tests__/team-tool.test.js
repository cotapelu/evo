...
describe('team_run tool', () => {
    ...
    describe('parameter validation', () => {
        it('should reject call reference strings with clear error', async () => {
            const result = await tool.execute(toolCallId, 'call_abc123', undefined, undefined, mockCtx);
        ...
        it('should reject invalid JSON strings', async () => {
            const result = await tool.execute(toolCallId, 'not valid json', undefined, undefined, mockCtx);
            expect(result.isError).toBe(true);
            expect(result.details?.error).toBe('Invalid JSON');
        });
        it('should reject object without tasks array', async () => {
            const result = await tool.execute(toolCallId, { teamSize: 2 }, undefined, undefined, mockCtx);
            expect(result.isError).toBe(true);
            expect(result.content[0].text).toContain('tasks must be a non-empty array');
        });
        it('should reject empty tasks array', async () => {
            const result = await tool.execute(toolCallId, { tasks: [] }, undefined, undefined, mockCtx);
            expect(result.isError).toBe(true);
            expect(result.content[0].text).toContain('tasks must be a non-empty array');
        });
        it('should accept valid object with tasks (may fail later due to runtime)', async () => {
            const result = await tool.execute(toolCallId, { tasks: ['ls'], teamSize: 1 }, undefined, undefined, mockCtx);
...
//# sourceMappingURL=team-tool.test.js.map