...
describe('AgentTeam', () => {
    ...
    beforeEach(() => {
        ...
        team.registerRuntime({ session: { id: 'parent-session-123' } }, 'parent');
        team.registerRuntime({ session: { id: 'agent1-session-456' } }, 'agent-1');
    });
    afterEach(async () => {
        if (team) {
            await team.dispose();
    ...
    describe('task management', () => {
        ...
        it('should isolate agents: agent-1 cannot complete agent-2 task', async () => {
            ...
            team.registerRuntime({ session: { id: 'agent2-session-789' } }, 'agent-2');
