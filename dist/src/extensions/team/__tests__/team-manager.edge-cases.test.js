...
describe('AgentTeam Edge Cases', () => {
    ...
    beforeEach(() => {
        team = new AgentTeam();
        team.registerRuntime({ session: { id: 'parent' } }, 'parent');
        team.registerRuntime({ session: { id: 'agent1' } }, 'agent-1');
        team.registerRuntime({ session: { id: 'agent2' } }, 'agent-2');
    });
    afterEach(async () => {
        if (team) {
            await team.dispose();
...
//# sourceMappingURL=team-manager.edge-cases.test.js.map