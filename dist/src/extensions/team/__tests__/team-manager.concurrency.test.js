...
const mockRuntime = (id) => ({ session: { id } });
    ...
    test('should handle mixed claim/complete concurrently', async () => {
        ...
        for (let round = 0; round < rounds; round++) {
            ...
            if (availableAgents.length === 0)
                break;
            const claims = await Promise.all(availableAgents.map(a => team.claimTask(a)));
            // Some agents might get null if no tasks left
            const allIndices = claims.filter(r => r !== null);
...
//# sourceMappingURL=team-manager.concurrency.test.js.map