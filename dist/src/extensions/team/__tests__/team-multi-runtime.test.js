...
function createSimpleRuntime(role) {
...
function createMockAgentSessionEvent(type, message) {
    return {
        type: type,
        ...(message && { message })
    };
...
describe('AgentTeam Multi-Runtime', () => {
    ...
    beforeEach(() => {
        ...
        parentRuntime = createSimpleRuntime('parent');
        // Configure parent's createRuntime to return a fresh child runtime each call
        parentRuntime.createRuntime = jest.fn().mockImplementation(async (options) => {
            return createSimpleRuntime('child');
        });
    ...
    describe('setupChildRuntimes', () => {
        it('should create correct number of child runtimes (teamSize=4)', async () => {
            ...
            await team.setupChildRuntimes(parentRuntime);
            ...
            for (const role of team.roles) {
                const runtime = team.runtimes.find((r) => team.roleByAgentId.get(r.session.sessionId) === role);
        ...
        it('should create child runtimes with custom agentCwd', async () => {
            ...
            await team.setupChildRuntimes(parentRuntime, '/custom/agent/cwd');
            const createCalls = parentRuntime.createRuntime.mock.calls;
        ...
        it('should create isolated session directories for each agent', async () => {
            ...
            const firstAgentDir = firstOpts.agentDir;
            const secondAgentDir = secondOpts.agentDir;
        ...
        it('should subscribe to child session events', async () => {
            ...
            await team.initialize(['task1']);
            await team.setupChildRuntimes(parentRuntime);
            const childRuntime = team.runtimes[0];
            const subscribe = childRuntime.session.subscribe;
    ...
    describe('agent loop integration', () => {
        ...
        it('should dispose child runtimes on team.dispose', async () => {
            ...
            const childRuntimes = team.runtimes;
    ...
    describe('concurrent event forwarding', () => {
        it('should handle multiple events from different agents concurrently', async () => {
            ...
            const roles = calls.map((call) => call[0].details.role);
            expect(roles).toContain('agent-1');
            expect(roles).toContain('agent-2');
        });
    });
});
//# sourceMappingURL=team-multi-runtime.test.js.map