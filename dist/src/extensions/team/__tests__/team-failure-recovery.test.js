...
describe('AgentTeam Failure Recovery', () => {
    ...
    beforeEach(async () => {
        ...
        team.agentStatuses.set('agent-2', { currentTaskIndex: null, status: 'idle' });
    });
    afterEach(async () => {
        const registry = TeamRegistry.getInstance();
        await team.dispose();
    ...
    test('should respect exponential backoff delay before task becomes claimable again', async () => {
        ...
        team.taskStatuses.get(0).retryAvailableAt = Date.now() - 1000;
    ...
    test('monitorInterval should not dispose team with pending or failed tasks', async () => {
        ...
        team.taskStatuses.set(0, { assignee: null, status: 'completed', result: 'done', retryCount: 0 });
        team.taskStatuses.set(1, { assignee: null, status: 'failed', result: 'error', retryCount: 3 });
    ...
    test('getTeamStatus should include failure metrics', async () => {
        ...
        team.taskStatuses.set(0, { assignee: null, status: 'completed', result: 'done', retryCount: 0 });
        team.taskStatuses.set(1, { assignee: null, status: 'failed', result: 'error', retryCount: 2 });
        team.taskStatuses.set(2, { assignee: 'agent-2', status: 'in_progress', result: '', retryCount: 0 });
        const status = await team.getTeamStatus();
...
//# sourceMappingURL=team-failure-recovery.test.js.map