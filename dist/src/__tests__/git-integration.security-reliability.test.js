...
function createMockPi() {
    const mockPi = {
        exec: jest.fn().mockImplementation(async (command, args, options) => {
...
function getHandler(mockPi, event) {
    return mockPi._handlers?.[event];
}
describe('Git Integration – Security & Reliability', () => {
    ...
    describe('execGit timeout option', () => {
        it('should pass timeout to pi.exec for isGitRepo call', async () => {
            ...
            const revParseCall = calls.find(([cmd, args]) => cmd === 'git' && args.includes('rev-parse'));
    ...
    describe('Retry logic', () => {
        it('should retry on failure up to maxRetries (2)', async () => {
            ...
            let stashAttempt = 0;
            mockPi.exec.mockImplementation(async (command, args, options) => {
            ...
            const stashCalls = mockPi.exec.mock.calls.filter(([cmd, args]) => cmd === 'git' && args.includes('stash'));
        ...
        it('should stop after maxRetries and throw', async () => {
            ...
            await toolResultHandler(null, { sessionManager: mockPi.sessionManager });
            mockPi.exec.mockImplementation(async (command, args) => {
                if (args.includes('rev-parse')) {
                    return { stdout: '', code: 0 };
                }
                if (args.includes('stash')) {
    ...
    describe('Stash message sanitization', () => {
        it('removes control characters and newlines', async () => {
            ...
            await Promise.resolve();
            const calls = mockPi.exec.mock.calls;
            const stashCall = calls.find(([cmd, args]) => cmd === 'git' && args.includes('stash') && args.includes('create'));
    ...
    describe('Checkpoint cleanup', () => {
        it('clears checkpoints on agent_end', async () => {
            ...
            await turnStartHandler();
            await Promise.resolve();
            expect(mockPi.exec.mock.calls.some(([cmd, args]) => cmd === 'git' && args.includes('stash'))).toBe(true);
    ...
    describe('hasUncommittedChanges error handling', () => {
        it('logs debug and returns false when git status fails', async () => {
            ...
            await shutdownHandler(null, ctx);
            await Promise.resolve();
            expect(debugSpy).toHaveBeenCalledWith(expect.stringContaining('Git status check failed:'), expect.any(String));
        ...
        it('produces a [pi] prefixed message when no assistant messages', async () => {
            ...
            mockPi.exec.mockImplementation(async (command, args) => {
                if (args.includes('rev-parse'))
                    return { stdout: '', code: 0 };
                if (args.includes('status'))
                    return { stdout: ' M file.txt', code: 0 };
            ...
            await shutdownHandler(null, ctx);
            await Promise.resolve();
            const commitCalls = mockPi.exec.mock.calls.filter(([cmd, args]) => cmd === 'git' && args.includes('commit'));
    ...
    describe('Checkpoint restore flow', () => {
        it('applies stash and notifies on success', async () => {
            ...
            await Promise.resolve();
            const applyCalls = mockPi.exec.mock.calls.filter(([cmd, args]) => cmd === 'git' && args.includes('apply'));
        ...
        it('notifies error on stash apply failure', async () => {
            ...
            await Promise.resolve();
            expect(mockPi.ui.notify).toHaveBeenCalledWith(expect.stringContaining('Failed to restore'), 'error');
        });
    });
});
//# sourceMappingURL=git-integration.security-reliability.test.js.map