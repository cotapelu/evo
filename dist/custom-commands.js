...
import { EvoPackageManager } from "./evo-package-manager.js";
import { logger } from "./utils/logger.js";
// ============ handlePinCommand ============
export async function handlePinCommand(args) {
    if (args[0] !== "pin")
        return;
    let local = false;
    let help = false;
    ...
    for (let i = 1; i < args.length; i++) {
        if (args[i] === "-l" || args[i] === "--local") {
            ...
            if (!oldSource) {
                ...
                logger.error(chalk.dim(`Usage: evo pin <oldSource> <newSource> [-l]`));
                process.exit(1);
            }
        }
        else {
    ...
    if (help) {
        logger.log(`
Usage: evo pin <oldSource> <newSource> [-l]
...
Options:
  -l, --local         Operate on project settings (.evo/settings.json)
  -h, --help          Show this help

Example:
  evo pin npm:chalk@1.0 npm:chalk@1.2
`);
        return;
    }
    if (!oldSource || !newSource) {
        logger.error(chalk.red("Missing arguments. Usage: evo pin <oldSource> <newSource> [-l]"));
        process.exit(1);
    }
    ...
    try {
        ...
        const idx = settings.packages.findIndex((p) => {
            if (typeof p === "string")
                return p === oldSource;
...
export async function handleExportCommand(args) {
    ...
    for (let i = 1; i < args.length; i++) {
        if (args[i] === "-l" || args[i] === "--local") {
            ...
            if (!outputFile) {
                ...
                logger.error(chalk.red(`Unexpected argument: ${args[i]}`));
                logger.error(chalk.dim(`Usage: evo export [output.json] [-l]`));
                process.exit(1);
            }
        }
        else {
    ...
    if (help) {
        logger.log(`
Usage: evo export [output.json] [-l]
...
Options:
  -l, --local         Export from project settings (.evo/settings.json)
  -h, --help          Show this help

Examples:
  evo export -l > packages.json
  evo export backup.json
`);
        return;
    }
    const cwd = process.cwd();
...
// ============ handleImportCommand ============
export async function handleImportCommand(args) {
    if (args[0] !== "import")
        return;
    let local = false;
    let help = false;
    ...
    for (let i = 1; i < args.length; i++) {
        if (args[i] === "-l" || args[i] === "--local") {
            ...
            if (!inputFile) {
                ...
                logger.error(chalk.red(`Unexpected argument: ${args[i]}`));
                logger.error(chalk.dim(`Usage: evo import <input.json> [-l]`));
                process.exit(1);
            }
        }
        else {
...
Options:
  -l, --local         Import into project settings (.evo/settings.json)
  -h, --help          Show this help

Examples:
  evo import packages.json -l
  cat packages.json | evo import -
`);
        return;
    }
    if (!inputFile) {
        logger.error(chalk.red("Missing input file. Usage: evo import <input.json> [-l]"));
        process.exit(1);
    }
    const cwd = process.cwd();
    const agentDir = getAgentDir();
    ...
    try {
        ...
        for (const pkg of packages) {
            ...
            const exists = existing.packages.some((p) => {
                if (typeof p === "string")
                    return p === source;
                if (typeof p === "object" && p !== null && "source" in p)
...
// ============ handleHealthCommand ============
export async function handleHealthCommand(args) {
    if (args[0] !== "health")
        return;
    const cwd = process.cwd();
    const agentDir = getAgentDir();
    try {
        const pm = new EvoPackageManager({ cwd, agentDir });
...
export async function handleCustomCommands(args) {
